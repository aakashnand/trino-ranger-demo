...
# HCOMPONENT_INSTALL_DIR is custom variable which is read from file install.properties
# to configure correct directory of plugins for trino in docker environment
...
# When trino is installed in docker, unlike normal non-docker installation, 
# the plugin and configuration directories for are different. INSTALL_ENV is 
# custom variable which is read from file install.properties to check 
# whether the installation environment for trino is docker or not. 
